...
final class ScheduleTestRequest implements ModuleInterface
{
    ...
    /**
     * @var \Staatic\WordPress\Request\TestRequest
     */
    public $testRequest;
    ...
    public function hooks() : void
    {
        ...
        \add_action(self::HOOK, [$this, 'dispatchTestRequest']);
        \add_action('wp_loaded', [$this, 'setupSchedule']);
    }

    public function setupSchedule() : void
    {
        if ($this->scheduler->isScheduled(self::HOOK)) {
            return;
        }
        $this->dispatchTestRequest();
        $this->scheduler->schedule(self::HOOK, self::SCHEDULE);
    }

    public function dispatchTestRequest() : void
    {
